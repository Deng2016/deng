...
def get_engine_abs_path(_edition: str, _arch: str, _version: str) -> str:
    """获取引擎绝对路径"""
    _app_path = os.environ["PROGRAMFILES"]
    if _arch.lower() == "x64":
        _app_path.replace(" (x86)", "")

    if _edition.lower() == "enterprise":
        _app_path += os.sep + "UiBot Enterprise"
    else:
        _app_path += os.sep + "UiBot Community"

    config_path = _app_path + os.sep + "config.json"
    config_info = file_utils.read_file_content(config_path, _return="json")
    version_no = config_info["currentVersion"]
    _deputy_abs_path = _app_path + os.sep + version_no + os.sep + "Deputy.exe"
    file_utils.check_path_is_exits(_deputy_abs_path, path_type="file")
    return _deputy_abs_path
